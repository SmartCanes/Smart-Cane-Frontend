...
const FEATURE_CARD_ACTIVE_CLASS = 'opacity-100 scale-100 sm:scale-[1.02] shadow-[0_40px_80px_rgba(9,20,46,0.45)]'
...

{
		...
		description: 'iCane\'s IoT integration links the device to a companion web application that provides live updates on battery level, connectivity, and user location for continuous monitoring.',
		...
		backgroundColor: '#122550',
		iconWrapperClassName: 'bg-transparent shadow-none text-white overflow-visible',
		iconWrapperWidth: '187.86px',
		iconWrapperHeight: '172.9px',	
		iconClassName: 'text-[160px]',
		inactiveClassName: FEATURE_CARD_INACTIVE_CLASS,
...

{
		...
		description: 'Stay one step ahead of the weather. iCane delivers instant weather alerts to help users prepare for outdoor conditions and ensure safer navigation.',
		...
		textColor: '#11285A',
		descriptionColor: '#1C253C',
		iconWrapperClassName: 'bg-transparent shadow-none text-[#11285A] overflow-visible',
		iconWrapperWidth: '187.86px',
		iconWrapperHeight: '172.9px',	
		iconClassName: 'text-[160px]',
		inactiveClassName: FEATURE_CARD_INACTIVE_CLASS,
...
const GuestPage = () => {
    ...
    index => {
        ...
        const clampedIndex = Math.max(0, Math.min(index, featureCards.length - 1));
        const targetCard = cardRefs.current[clampedIndex];
        if (!targetCard) return;

        // --- ITO ANG MAS SIMPLE AT TAMANG PAGKAKALKULA ---
        
        // 1. Kunin ang lapad ng container (yung scrollable area)
        const containerWidth = container.offsetWidth;

        // 2. Kunin ang lapad at pwesto ng card na pupuntahan
        const cardWidth = targetCard.offsetWidth;
        const cardLeft = targetCard.offsetLeft; // Posisyon ng card sa loob ng container

        // 3. Kalkulahin ang bagong scroll position para ma-center ang card
        const newScrollPosition = cardLeft - (containerWidth / 2) + (cardWidth / 2);

        // 4. I-scroll ang container sa bagong pwesto
        container.scrollTo({
			...
			// Update active card state after scroll
			window.requestAnimationFrame(() => {
				setTimeout(updateActiveCard, 180);
			});
		},
		[updateActiveCard, featureCards.length]
	)

	const handleCarouselScroll = direction => {
		const targetIndex = activeCardIndex + direction
	...
	useEffect(() => {
		...
		const intervalId = setInterval(() => {
			scrollToCard(activeCardIndex === featureCards.length - 1 ? 0 : activeCardIndex + 1)
		}, 5000)
	...
	return (
		<div className='min-h-screen w-full bg-[#FDFCF9] text-[#1C253C] font-poppins'>
			...
			<header className='w-full bg-white/95 backdrop-blur shadow-sm sticky top-0 z-20'>
				<div className='max-w-6xl mx-auto flex items-center justify-between px-4 py-3 sm:px-6 sm:py-4'>
					<div className='flex items-center gap-3'>
						...
						<img src={icaneLogo} alt='iCane logo' className='h-11 w-11 object-contain' />
						
						
					</div>

					<nav className='hidden md:flex items-center gap-10 font-montserrat text-sm  tracking-wide'>
						<a href='#home' className='hover:text-[#11285A] transition-colors duration-200'>Home</a>
						<a href='#features' className='hover:text-[#11285A] transition-colors duration-200'>iCane</a>
						<a href='#about' className='hover:text-[#11285A] transition-colors duration-200'>About Us</a>
						<a href='#contact' className='hover:text-[#11285A] transition-colors duration-200'>Contact Us</a>
					</nav>
			...
			>
					...
					<div className='relative mt-12'>
						<button
							...
							style={{ opacity: isFirstCardActive ? 0.3 : 1, pointerEvents: isFirstCardActive ? 'none' : 'auto' }}
						>
							<Icon icon='mingcute:left-line' className='text-5xl transition-transform duration-200 group-hover:-translate-x-0.5' />
						</button>

						<div
							ref={carouselRef}
							onScroll={updateActiveCard}
							className='flex snap-x snap-mandatory gap-6 overflow-x-auto pb-6 pt-4 scroll-smooth sm:gap-8 lg:gap-10 [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden'
							style={{ scrollPaddingLeft: 'min(10%, 72px)', scrollPaddingRight: 'min(10%, 72px)' }}
						>
							{featureCards.map((card, index) => (
								<FeatureCard
									...
									className={`snap-center transition-all duration-300 ease-out ${card.className ?? ''}`}
									activeClassName={card.activeClassName ?? FEATURE_CARD_ACTIVE_CLASS}
									inactiveClassName={card.inactiveClassName ?? FEATURE_CARD_INACTIVE_CLASS}
								/>
							))}
						</div>
						...
						<button
							...
							style={{ opacity: isLastCardActive ? 0.3 : 1, pointerEvents: isLastCardActive ? 'none' : 'auto' }}
						>
							<Icon icon='mingcute:right-line' className='text-5xl transition-transform duration-200 group-hover:translate-x-0.5' />
						</button>
					</div>
						...
						{featureCards.map((card, index) => (
							<button
								...
								className={`h-2.5 rounded-full transition-all duration-200 focus-visible:ring-2 focus-visible:ring-[#11285A] focus-visible:ring-offset-2 ${
									activeCardIndex === index ? 'w-8 bg-[#11285A]' : 'w-2.5 bg-[#d7dde9]'
								}`}
			...
			<section id='icane' className='scroll-mt-24 px-4 sm:px-6'>
				<div className='relative mx-auto flex max-w-5xl items-center justify-center py-12 md:py-16'>
			...
			<section id='about' className='scroll-mt-24 px-4 sm:px-6'>
				<div className='relative mx-auto flex max-w-5xl items-center justify-center py-12 md:py-16'>
					<div className='h-px w-full bg-[#bfcef0]' aria-hidden='true' />
					<h3 className='absolute bg-[#FDFCF9] px-12 text-[10px] font-semibold tracking-[0.5em] text-[#11285A] sm:px-20'>
						ABOUT US
		...
		<footer className='bg-[#11285A] text-white'>
				...
				<div className=' border-white/15 pt-10 grid gap-10 md:grid-cols-4'>
					<div className='space-y-4'>
						...
						<p className='text-sm leading-relaxed text-white/70'>
							A smart mobility cane empowering the visually impaired with safety, guidance, and connectivity.
						</p>
					</div>
					<div className='space-y-4'>
						<h4 className='font-montserrat text-sm font-semibold tracking-[0.2em] text-white/80'>Category</h4>
						<nav className='space-y-3 text-sm text-white/70'>
							<a href='#home' className='block hover:text-white transition-colors duration-200'>Home</a>
							<a href='#features' className='block hover:text-white transition-colors duration-200'>iCane</a>
							<a href='#about' className='block hover:text-white transition-colors duration-200'>About Us</a>
							<a href='#contact' className='block hover:text-white transition-colors duration-200'>Contact Us</a>
						</nav>
					</div>
					<div className='space-y-4'>
						<h4 className='font-montserrat text-sm font-semibold tracking-[0.2em] text-white/80'>Contact Info</h4>
					...
					<div className='space-y-4'>
						<h4 className='font-montserrat text-sm font-semibold tracking-[0.2em] text-white/80'>Social Media</h4>
						<div className='flex flex-wrap gap-3'>
							<a href='#!' aria-label='Visit the iCane Facebook page' className='inline-flex h-10 w-10 items-center justify-center'>
								<img src={facebookIcon} alt='Facebook icon' className='h-10 w-10 object-contain' />
							</a>
							<a href='#!' aria-label='Visit the iCane Twitter page' className='inline-flex h-10 w-10 items-center justify-center'>
								<img src={twitterIcon} alt='Twitter icon' className='h-10 w-10 object-contain' />
							</a>
							<a href='#!' aria-label='Visit the iCane Instagram page' className='inline-flex h-10 w-10 items-center justify-center'>
								<img src={instagramIcon} alt='Instagram icon' className='h-10 w-10 object-contain' />
							</a>
						</div>
						<a href='#faq' className='block text-sm text-white/70 hover:text-white transition-colors duration-200'>FAQ</a>
					</div>
				</div>
			...
			<div 
				className=' w-full h-[0.5px] bg-[#dfdfdf]' 
