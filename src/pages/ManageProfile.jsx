...
import { createVIP, getMyVIP, updateMyVIP, deleteVIP, uploadVIPImage } from "@/api/backendService.js";
...
// ===========================================
  ...
  // ========== VIP COMPONENTS ==========
  const ProfileInfo = ({ label, value, isEditing, onChange, name, disabled = false }) => {
    return (
      <div>
  ...
  const validateName = (name) => {
    ...
    const nameRegex = /^[A-Za-z\s\-\'\.]+(?:\s+(?:Jr\.?|Sr\.?|I{1,3}|IV|V|VI|VII|VIII|IX|X))?$/i;
    ...
    for (let word of words) {
      if (word.length === 2 && word.endsWith('.') && /^[A-Za-z]\.$/.test(word)) {
        continue;
      }
  ...
  const handleAddVIPProfile = () => {
    if (hasVIPProfile) {
      setToast({
        show: true,
        type: "info",
        message: "You already have a VIP profile. Edit your existing profile instead."
      });
      return;
    }
    ...
    try {
      ...
      if (response && response.data) {
        ...
        if (imageFile && newVipId) {
          const uploadedUrl = await uploadVIPImageToBackend(newVipId, imageFile);
          if (uploadedUrl) {
            await updateMyVIP({
              avatar: uploadedUrl
            });
      ...
      setToast({
        show: true,
        type: "error",
        message: error.response?.data?.message || "Failed to create VIP profile. Please check all required fields."
      });
    } finally {
      setIsLoading(false);
    }
      ...
      return (
        <div className="flex justify-center items-center min-h-[400px]">
          <div className="text-center">
            <Icon icon="eos-icons:loading" className="w-12 h-12 text-blue-500 mx-auto mb-4" />
                  ...
                  <div className="flex flex-col gap-2">
                    <div>
                      ...
                      <p className="text-xs text-gray-500 mt-1">
                        Upload a new profile image. Maximum file size: 2MB. Supported formats: JPG, PNG, GIF.
                      </p>
                    </div>
                    ...
                    {editImageFile && (
                      <p className="text-xs text-gray-500 mt-1">
                        New photo selected: {editImageFile.name} ({(editImageFile.size / 1024 / 1024).toFixed(2)} MB)
                      </p>
                    )}
                  </div>
                </div>
              ...
              <div className="flex flex-col gap-2">
                <div className="flex flex-col md:flex-row items-start md:items-center gap-4 p-4 border border-gray-200 rounded-lg bg-gray-50">
                  ...
                  <div className="flex-1">
                    <div className="flex flex-col gap-2">
                      <div>
                        <h4 className="text-sm font-semibold text-gray-700">Profile Photo (Optional)</h4>
                        <p className="text-xs text-gray-500 mt-1">
      ...
      <div className="mx-auto w-full space-y-6 sm:space-y-8">
        ...
        {vipDeleteConfirm.show && (
          <Modal
            ...
            handleCancel={() => setVipDeleteConfirm({ show: false, vipId: null })}
