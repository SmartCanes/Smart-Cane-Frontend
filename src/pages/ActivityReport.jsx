...
const ACTION_OPTIONS = [
  ...
  { value: "REMOVE_GUARDIAN",     label: "Remove Guardian",   icon: "ph:user-minus-bold" },
...
const ActivityLogs = () => {
  ...
  const renderGuardianAvatar = (activity, size = "w-10 h-10") => {
    ...
    return (
      <div className={`${size} rounded-full overflow-hidden shrink-0 bg-gray-100`}>
        {avatar ? (
          <img loading="lazy" src={resolveProfileImageSrc(avatar)} alt={guardianName} className="w-full h-full object-cover" />
        ) : (
          <DefaultProfile bgColor="bg-primary-100" userInitial={guardianName.charAt(0)} />
        )}
      </div>
    );
  };
      ...
      <main className="bg-white md:bg-[#f9fafb] rounded-t-[32px] md:rounded-none min-h-[calc(100vh-var(--header-height)-var(--mobile-nav-height))] md:min-h-[calc(100vh-var(--header-height))] md:max-h-[calc(100vh-var(--header-height))] overflow-y-visible md:overflow-y-auto p-6 pb-[calc(var(--mobile-nav-height)+1.5rem)] md:pb-6">
        <div className="w-full font-poppins max-w-5xl mx-auto space-y-6">
          <div className="mb-4 md:mb-8">
            <h2 className="text-xl sm:text-2xl font-bold text-gray-800">Activity Logs</h2>
            <p className="text-gray-500 text-xs md:text-sm">Monitor and track all guardian activities in your system</p>
          </div>
    ...
    <main className="bg-white md:bg-[#f9fafb] rounded-t-[32px] md:rounded-none min-h-[calc(100vh-var(--header-height)-var(--mobile-nav-height))] md:min-h-[calc(100vh-var(--header-height))] md:max-h-[calc(100vh-var(--header-height))] overflow-y-visible md:overflow-y-auto p-6 pb-[calc(var(--mobile-nav-height)+1.5rem)] md:pb-6">
      <div className="w-full font-poppins max-w-5xl mx-auto space-y-6 sm:space-y-8 md:max-w-none md:mx-0 md:pr-6">
        <div data-tour="tour-activity-header" className="mb-4 md:mb-8">
          <h2 className="text-xl sm:text-2xl font-bold text-gray-800 text-nowrap">Activity Logs</h2>
          <p className="text-gray-500 text-xs md:text-sm">Monitor and track all guardian activities in your system</p>
        </div>
              ...
              {selectedActions.map((val) => {
                ...
                return (
                  <span key={val} className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-[#11285A]/10 text-[#11285A] text-xs font-medium">
                    ...
                    {opt?.label || val}
                    <button onClick={() => handleActionChange(val)} className="ml-0.5 hover:text-red-500 cursor-pointer">
        ...
        {!isLoading && totalItems === 0 ? (
          <div className="bg-white rounded-b-2xl shadow-sm p-8 md:p-16 flex flex-col items-center justify-center text-center">
            ...
            <h3 className="text-lg font-semibold text-[#11285A] mb-1">
              {searchTerm || selectedActions.length > 0 ? "Nothing to see here" : "No activity yet"}
            </h3>
            ...
            {(searchTerm || selectedActions.length > 0) && (
              <button onClick={() => { setSearchTerm(""); handleClearAll(); }} className="mt-6 text-sm font-medium text-blue-600 hover:underline">
                Clear filters
              </button>
            )}
          </div>
          ...
          <>
            ...
            <div className="hidden md:block bg-white rounded-b-2xl shadow-sm overflow-hidden">
                    ...
                    {isLoading
                      ? Array.from({ length: itemsPerPage }).map((_, index) => (
                          <tr key={`sk-${index}`} className="animate-pulse">
                            <td className="py-4 px-6"><div className="flex items-center gap-3"><div className="w-10 h-10 bg-gray-200 rounded-full" /><div className="h-4 w-32 bg-gray-200 rounded" /></div></td>
                            <td className="py-4 px-6"><div className="h-7 w-24 bg-gray-200 rounded-full" /></td>
                            <td className="py-4 px-6"><div className="space-y-2"><div className="h-4 w-full bg-gray-200 rounded" /><div className="h-4 w-4/5 bg-gray-200 rounded" /></div></td>
                            <td className="py-4 px-6 text-right"><div className="h-4 w-28 bg-gray-200 rounded ml-auto" /></td>
                          </tr>
                        ))
                      : paginatedActivities.map((activity, index) => (
                          <tr key={activity.historyId ?? index} className="hover:bg-gray-50 transition-colors">
                            ...
                            <td className="py-4 px-6"><p className="text-sm text-gray-600">{activity.description || "—"}</p></td>
              ...
              <div className="flex items-center justify-between px-6 py-4 border-t border-gray-100">
                ...
                <div className="flex items-center gap-2">
                  <button onClick={() => handlePageChange(currentPage - 1)} disabled={currentPage === 1} className="w-8 h-8 flex items-center justify-center rounded-lg border border-gray-200 text-gray-500 hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed"><Icon icon="ph:caret-left" /></button>
                  {getVisiblePages().map((page, index) =>
                    typeof page === "number" ? (
                      <button key={index} onClick={() => handlePageChange(page)} className={`w-8 h-8 flex items-center justify-center rounded-lg font-medium text-sm transition-colors ${currentPage === page ? "bg-[#11285A] text-white" : "border border-gray-200 text-gray-600 hover:bg-gray-50"}`}>{page}</button>
                    ) : (
                      <span key={index} className="text-gray-400 px-1">...</span>
                    )
                  )}
                  <button onClick={() => handlePageChange(currentPage + 1)} disabled={currentPage === totalPages} className="w-8 h-8 flex items-center justify-center rounded-lg border border-gray-200 text-gray-500 hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed"><Icon icon="ph:caret-right" /></button>
                </div>
              </div>
            </div>
            ...
            <div className="md:hidden bg-white rounded-b-2xl shadow-sm overflow-hidden">
                  ...
                  <div key={activity.historyId ?? index} className="p-4 hover:bg-gray-50 transition-colors">
                    <div className="flex items-start gap-3 mb-3">
                      ...
                      <div className="min-w-0 flex-1">
                        <p className="font-semibold text-sm text-gray-900 truncate">{activity.guardianName || "—"}</p>
                        <p className="text-xs text-gray-400 font-medium mt-1">{activity.createdAt ? activity.createdAt.replace("T", " ").split(".")[0] : "—"}</p>
                      </div>
                    </div>
              ...
              <div className="flex flex-col sm:flex-row items-center justify-between gap-3 px-4 py-4 border-t border-gray-100 bg-gray-50">
                <p className="text-xs text-gray-500">Showing {Math.min((currentPage - 1) * itemsPerPage + 1, totalItems)}–{Math.min(currentPage * itemsPerPage, totalItems)} of {totalItems.toLocaleString()}</p>
                <div className="flex items-center gap-1.5">
                  ...
                  {getVisiblePages().map((page, index) =>
                    typeof page === "number" ? (
                      <button key={index} onClick={() => handlePageChange(page)} className={`w-8 h-8 flex items-center justify-center rounded-lg font-medium text-xs transition-colors ${currentPage === page ? "bg-[#11285A] text-white" : "border border-gray-200 bg-white text-gray-600 hover:bg-gray-50"}`}>{page}</button>
                    ) : (
