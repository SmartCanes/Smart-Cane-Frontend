...
const GuardianNetwork = () => {
  ...
  <div className="bg-white rounded-2xl shadow-sm p-6 w-full font-poppins">
      ...
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-xl font-semibold text-gray-800">Guardian Network</h3>
          ...
          <div key={index} className="bg-white border border-gray-200 rounded-xl p-4 flex items-center gap-4">
            ...
            <div>
              ...
              <div className="flex items-center gap-1.5 mt-1">
                <span className={`w-2 h-2 rounded-full ${guardian.status === 'Active' ? 'bg-green-500' : 'bg-gray-400'}`}></span>
                <p className={`text-xs font-medium ${guardian.status === 'Active' ? 'text-green-600' : 'text-gray-500'}`}>
                  {guardian.status}
                </p>
              </div>
            </div>
